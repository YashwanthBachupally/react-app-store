...
class AppStore extends Component {
  state = {
    searchinput: '',
    tabListId: tabsList[0].tabId,
    eachCategoryApp: appsList,
  }
  ...
  changeTab = tabId => {
    this.setState({tabListId: tabId})
  }

  render() {
    const {tabListId, searchinput, eachCategoryApp} = this.state

    const filteredApps = eachCategoryApp.filter(
      each =>
        tabListId === each.category &&
      ...
      <div className="app-cont">
        <div className="items-cont">
          ...
          <div className="search-cont">
            <input type="search" onChange={this.searched} />
            <img
          ...
          <ul className="tabItem">
            {tabsList.map(each => (
              <TabItem
                ...
                isActiveTav={each.tabId === tabListId}
              />
            ))}
          </ul>
